...
import com.jxy.api.model.dto.user.UserUpdateRequest;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    @Override
    public UserVO updateUserVoucher(UserUpdateRequest userUpdateRequest, HttpServletRequest request) {
        User loginUser = getLoginUser(request);
        if (loginUser == null) {
            throw new BusinessException(ErrorCode.NOT_LOGIN_ERROR);
        }
        String userAccount = loginUser.getUserAccount();
        String encryptPassword = loginUser.getUserPassword();
        User user = new User();
        user.setId(loginUser.getId());
        // 更新ak、sk
        String ak = Base64Utils.encodeToString((SALT + userAccount + RandomStringUtils.random(4)).getBytes());
        user.setAccessKey(ak);
        String sk = Base64Utils.encodeToString((SALT + encryptPassword + RandomStringUtils.random(8)).getBytes());
        user.setSecretKey(sk);
        boolean b = updateById(user);
        if (!b) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "更新凭证失败，请重试！");
        }
